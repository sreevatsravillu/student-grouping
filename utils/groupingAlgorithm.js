class GroupingAlgorithm {
  ...
  let y1 = [];
  let z1 = []
  console.log(sortedEntries,"+++++++++",filteredEntries)
  filteredEntries.forEach(entry => {
    if(!entry[1].priority){
      ...
      x1.push(entry[1].uid); 
      // For y, push the group name(s) (duplicated based on uid count)
      y1.push(Array(entry[1].uid.length).fill(entry[0]));
      // For z, push the name array(s) (duplicated based on uid count)
      z1.push(entry[1].name)

    }
  });
  // Sort arrays for priority 1 based on length (largest first)
  x1.sort((a, b) => b.length - a.length);
  y1.sort((a, b) => b.length - a.length);
  z1.sort((a, b) => b.length - a.length);
